...
/**
 * Normalise an arbitrary string into a single, cross-platform-safe download-filename
 * PART (no extension). Strips illegal characters, trims leading/trailing dots & spaces,
 * collapses whitespace to single dashes, and caps length.
 */
 ...
 * Returns null for `var(...)` references, `none`/`transparent`, translucent fills, and named colours.
 */
...
/**
 * Given an explicit `body.fill`, the label + subtitle colours that contrast it.
 * Returns null when the body is theme-adaptive/translucent.
 */
export function nodeContrastText(bodyFill) {
  ...
  return lum > 0.6
    ? { label: '#1C1E21', subtitle: 'rgba(0, 0, 0, 0.55)' }
    : { label: '#F5F6F7', subtitle: 'rgba(255, 255, 255, 0.6)' };
}
